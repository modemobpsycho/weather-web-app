...
export function HomeHeader({ forecast }: { forecast: IForecastHeader }) {
	...
	return (
		<div
			className={`flex flex-col items-center py-4 mt-4 w-full h-full z-20 ${
				isDay ? 'day-header' : 'night-header'
			}`}
		>
			<div className='flex flex-col items-center container-blur gap-2'>
				<p className='text-5xl font-light'>{forecast.location.name}</p>
				...
				<p className='text-2xl'>
					{getDayOfWeek(forecast.forecast.forecastday[0].date)}
				</p>
